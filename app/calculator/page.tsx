...
// ─────────────────────────────────────────────────────────────
// Live syringe diagram. Re-scales when the barrel changes and fills as the
// user enters a dose. Sits in its own dark "display" panel so it stands out.
...
function SyringeDiagram({
  ...
  // Geometry in viewBox units. Inner measurable barrel runs innerX..innerX+innerW.
  const innerX = 30;
  const innerW = 256;
  ...
  return (
    <div className="rounded-xl bg-zinc-900 p-4 shadow-sm">
      ...
      <p className="mt-2 text-center text-xs text-zinc-300">
        {hasDraw ? (
          over ? (
            <>Your dose is bigger than this {shortLabel} holds. Pick a larger syringe below.</>
          ) : (
            <>
              Fill to{' '}
          ...
          <>Enter your dose below and Buddy will show where to fill.</>
        )}
      </p>
    </div>
  );
